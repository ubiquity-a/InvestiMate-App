...
def calculate_budget(salary):
    ...
    return expenses, wishes, investments, development
...
def main():
    ...
    # Main page content
    st.set_page_config(page_title='FinFriend - Your InvestMate', page_icon= " :chart_with_upwards_trend:")
    st.title("**Finance Friend** - _Your InvestMate_ :moneybag:")
    st.write("---")
    ...
    if option == 'Budget Calculator':
        ...
        if salary:
            expenses, wishes, investments, development = calculate_budget(salary)
            st.write('### Budget Allocation')
            st.write(f'*50% for* `Expenses`: **₹{expenses:.2f}**')
            st.write(f'*20% for* `Wishes`: **₹{wishes:.2f}**')
        ...
        st.write('### About')
        st.markdown('''
        *FinFriend* is developed by **Akanksha** (Web developer, Python developer)

        * Gmail - akku902694@gmail.com
        * Connect me on LinkedIn - https://www.linkedin.com/in/akanksha-73b5a4232
